...
let rec expand_string_aux ?(regex=path_interp_regex) ?(escape_string=fun x -> x) ?(partial=false) ?default env text =
  ...
  let f g =
    ...
    if str = "%%" then (if partial then "%%" else "%")
      ...
      (if str.[1] = '<' then
         expand_string_aux ~regex:string_interp_regex ?escape_string:None ~partial ?default env (String.sub str 2 (String.length str - 4))
...
let arguments env (a,f) =
  if opt_eval_to_bool env f then
    ...
    | CIdent i ->
      ...
      | Some (L sl) -> sl
      | None -> OpamConsole.warning "ERR in replacement: undefined ident %S" i; [""]
  else
    []
