...
module OPAM: sig
  ...
  type t = private {
    ...
    env        : (spf_unresolved, ident_or_value) env_update list;
    ...
    build_env  : (spf_unresolved, ident_or_value) env_update list;
  ...
  val build_env: t -> (spf_unresolved, ident_or_value) env_update list
  ...
  val env: t -> (spf_unresolved, ident_or_value) env_update list
  ...
  val with_build_env: (spf_unresolved, ident_or_value) env_update list -> t -> t
  ...
  val with_env: (spf_unresolved, ident_or_value) env_update list -> t -> t
...
module Environment: IO_FILE with type t = (spf_resolved, string) env_update list
...
module Comp: sig
  ...
  val create_preinstalled:
    compiler -> compiler_version -> name list -> (spf_unresolved, string) env_update list -> t
  ...
  (** Environment variable to set-up before running commands in the
      subtree *)
  val env: t -> (spf_unresolved, string) env_update list
...
module Switch_config: sig
  type t = {
    ...
    wrappers: Wrappers.t;
    env: (spf_resolved, ident_or_value) env_update list;
