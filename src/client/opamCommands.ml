    ...
    mk_flag ~section:global_option_section ["no-self-upgrade"]
      (Printf.sprintf
        "Opam will replace itself with a newer binary found \
         at $(b,OPAMROOT%sopam) if present. This disables this behaviour." Filename.dir_sep |> Cmdliner_manpage.escape) in
...
let init =
  ...
  let man = [
    ...
    `P (Printf.sprintf
         ...
         Filename.dir_sep |> Cmdliner_manpage.escape);
  ...
  let no_config_file =
    mk_flag ["no-opamrc"]
      ...
      "Don't read `/etc/opamrc' or `~%s.opamrc': use the default settings and \
       the files specified through $(b,--config) only" Filename.dir_sep |> Cmdliner_manpage.escape)
  in
  let reinit =
    mk_flag ["reinit"]
      ...
      (match OpamStateConfig.get_switch_opt () with
       ...
       | Some sw ->
         `Ok (OpamConfigCommand.env gt sw
                ...
                ~cmd:(shell=SH_cmd || shell=SH_clink) ~csh:(shell=SH_csh) ~sexp ~fish:(shell=SH_fish) ~inplace_path))
    | Some `revert_env, [] ->
      OpamGlobalState.with_ `Lock_none @@ fun gt ->
      (match OpamStateConfig.get_switch_opt () with
       | None -> `Ok ()
       | Some sw ->
         `Ok (OpamConfigCommand.ensure_env gt sw;
              OpamConfigCommand.print_eval_env
                ~cmd:(shell=SH_cmd || shell=SH_clink) ~csh:(shell=SH_csh) ~sexp ~fish:(shell=SH_fish)
                (OpamEnv.add [] [])))
    | Some `exec, (_::_ as c) ->
      OpamGlobalState.with_ `Lock_none @@ fun gt ->
...
let switch =
  ...
  let man = [
    ...
    `P (Printf.sprintf
        ...
        Filename.dir_sep OpamSwitch.external_dirname |> Cmdliner_manpage.escape);
    ...
    mk_flag ["c"; "download-cache"]
      (Printf.sprintf
        "Clear the cache of downloaded files (\\$OPAMROOT%sdownload-cache), as \
         well as the obsolete \\$OPAMROOT%sarchives, if that exists." Filename.dir_sep Filename.dir_sep |> Cmdliner_manpage.escape)
  in
  let repos =
