...
let load_and_verify_env ~set_opamroot ~set_opamswitch ~force_path gt switch env_file =
  let upd =
    OpamEnv.get_opam_raw_updates ~set_opamroot ~set_opamswitch ~force_path gt.root switch
  in
  let environment_opam_switch_prefix =
    List.find_opt (function "OPAM_SWITCH_PREFIX", OpamParserTypes.Eq, _, _ -> true | _ -> false) upd
