...
val get_full:
  ...
  ?updates: ('r, string) env_update list -> ?scrub:string list -> 'a switch_state -> env
...
val compute_updates: ?force_path:bool -> 'a switch_state -> (spf_resolved, string) env_update list
...
(** Returns the full environment with only the PATH variable updated, as per
    [path] *)
val full_with_path:
  force_path:bool -> ?updates:(spf_resolved, string) env_update list -> dirname -> switch -> env

(** Performs variable expansion on the strings in an environment update *)
val env_expansion: ?opam:OpamFile.OPAM.t -> 'a switch_state -> ('r, string) env_update -> ('r, string) env_update

(** XXX *)
val expand_lists: 'a switch_state -> ?opam:OpamFile.OPAM.t -> (spf_resolved, ident_or_value) env_update list -> (spf_resolved, string) env_update list
