...
let shell_eval_string ?(root="") ?(switch="") ?(set_opamswitch=false) shell =
...
let eval_string gt ?set_opamswitch switch =
  let root =
  ...
  in
  shell_eval_string (OpamStd.Sys.guess_shell_compat ()) ~root ~switch ?set_opamswitch
...
let variables_clink = "opam env --clink"
let init_sh        = "init.sh"
...
let string_of_update st shell updates =
  ...
  let aux (ident, symbol, string, comment) =
    ...
    in
    let prefix, string =
      if OpamStd.Sys.is_windows && ident = "MANPATH" then
  ...
  let variables =
    match shell with
      `cmd ->
        Some "opam env"
    | `clink ->
        Some "os.execute(\"opam env --clink\")"
    | _ ->
...
let write_static_init_scripts root ~completion =
  let scripts =
    let shells =
      ...
      if OpamStd.Sys.is_windows then
        [
          `cmd, init_cmd, (variables_cmd, None);
          `clink, init_clink, (variables_clink, Some complete_clink);
        ]
      else
        [
    ...
    List.map (fun (shell, init, scripts) ->
        init, init_script root ~shell ~completion scripts) shells @ scripts
  in
    ...
    let f f =
      if shell = `cmd then
        let value = source root ~shell (init_file shell) in
          ...
          OpamStubs.(writeRegistry HKEY_CURRENT_USER (Filename.dirname f) (Filename.basename f) REG_SZ value);
          false
      else
...
let setup_interactive root ~dot_profile shell =
  let update dot_profile =
    OpamConsole.msg "\n";
    if update_user_setup_aux root ?dot_profile shell && shell = `clink then
      OpamConsole.msg
        "\n\
