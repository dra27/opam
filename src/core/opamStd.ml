...
module Win32 = struct
  ...
  external sendMessageTimeout : nativeint -> int -> int -> ('a, 'b, 'c) winmessage -> 'a -> 'b -> int * 'c = "OPAMW_SendMessageTimeout_byte" "OPAMW_SendMessageTimeout"
  ...
  let persistHomeDirectory home =
    ...
    ignore (sendMessageTimeout 0xffffn 5000 0x2 WM_SETTINGCHANGE 0 "Environment")
end
  ...
  let search_path_for_command ?(env=Env.initial_env) name =
    ...
    let path =
      List.find (fun path ->
        ...
        (get_path_dirs (Env.env_var env "PATH"))
    in
