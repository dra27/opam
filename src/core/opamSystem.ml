...
let copy src dst =
  ...
  command ~verbose:(verbose_for_base_commands ()) ("cp"::(cygify [src; dst ]))

let mv src dst =
  if Sys.file_exists dst then remove_file dst;
  mkdir (Filename.dirname dst);
  command ~verbose:(verbose_for_base_commands ()) ("mv"::(cygify [src; dst ]))
...
module Tar = struct
  ...
  let extract_function =
    let f = lazy (
      if OpamStd.(Sys.os () = Sys.Win32) && OpamStd.Sys.is_cygwin_variant (get_actual_command "tar") = `Cygwin then
          apply_cygpath
      else
        fun x -> x)
    in
