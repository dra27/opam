...
let ocamls =
  let ocamls =
    ["4.03.0"; "4.04.2"; "4.05.0"; "4.06.1"; "4.07.1"; (* Legacy *)
     "4.08.1"; "4.09.1"; "4.10.2"; "4.11.2"; "4.12.1"; "4.13.1"] in (* Current *)
...
{|    - name: opam bootstrap cache
      ...
      with:
        ...
        key: opam-%s${{ runner.os }}-${{ env.OPAMBSVERSION }}-${{ matrix.ocamlv }}-${{ env.OPAM_REPO_SHA }}-${{ hashFiles ('.github/scripts/main/opam-bs-cache.sh', '.github/scripts/main/preamble.sh') }}
...
{|      id: ocaml-cache
      ...
      with:
        ...
        key: ${{ runner.os }}-ocaml-%s-${{ hashFiles ('.github/scripts/main/ocaml-cache.sh', '.github/scripts/main/preamble.sh') }}%s
