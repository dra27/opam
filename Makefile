...
opam-admin.top: $(JBUILDER_DEP)
	$(JBUILDER) build $(JBUILDER_ARGS) src/tools/opam_admin_topstart.bc
...
clean:
	$(MAKE) -C doc $@
	rm -f opam$(EXE) opam-admin.top$(EXE) opam-installer$(EXE)
...
distclean: clean clean-ext
	...
	rm -f src/*.META src/*/.merlin src/stubs/jbuild src/tools/opam-putenv.inc src/client/manifest.inc src/client/opamManifest.inc
...
.PHONY: win-compilers win-builds win-zips
