...
#   4. Change DISTRIB to install to the build directory (i.e. don't install)
#   5. Configure Tcl/Tk
...
sed -i -e "/GetOcamlPath/d" \
       -e "s/OcamlPath.*[^;]/& = \"$(echo $3| sed -e "s/\\\\/\\\\\\\\\\\\\\\\/g")\\\\\\\\bin\\\\\\\\ocaml.exe\"/" \
       -e "s/LibDir.*[^;]/& = \"$(echo $3| sed -e "s/\\\\/\\\\\\\\\\\\\\\\/g")\\\\\\\\lib\\\\\\\\ocaml\"/" win32caml/ocaml.c
