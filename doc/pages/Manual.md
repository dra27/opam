...
Default separator is `";" | ";" { os = "win32"}` (`;` on Windows, `:` on Cygwin or any other system) and default format is `target` (no translation).

For example:
- `[FOO false]`: `FOO` won't be translated at all, and default separator is used
- `[FOO ":" "target-quoted"]: `FOO` will be append using `:` separator, and if the added path contains a `:`, it will be quoted
  - `FOO += "a/path/to"` -> FOO=a/path/to:previous/path
  - `FOO += "a:path/to"` -> FOO="a:path/to":previous/path
- `[FOO ";" "target-quoted"]: `FOO` will be append using `;` separator, and if the added path contains a `;`, it will be quoted
  - `FOO += "a/path/to"` -> FOO=a/path/to;previous/path
  - `FOO += "a;path/to"` -> FOO="a;path/to";previous/path
- `[FOO ";" "host"]: `FOO` will be append using `:`, and its path will be translated according host translator, on Windows:
  - `FOO += "/a/path/to"` -> FOO=C:\cygwin\a\path\to:C:\previous\path
