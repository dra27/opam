...
[dnl
  ...
  if test "$OCAMLC" != "no"; then
     OCAMLVERSION=`$OCAMLC -v | tr -d '\r' | sed -n -e 's|.*version* *\(.*\)$|\1|p'`
	...
	TMPVERSION=`$OCAMLOPT -v | tr -d '\r' | sed -n -e 's|.*version* *\(.*\)$|\1|p' `
	...
	TMPVERSION=`$OCAMLCDOTOPT -v | tr -d '\r' | sed -n -e 's|.*version* *\(.*\)$|\1|p' `
	if test "$TMPVERSION" != "$OCAMLVERSION" ; then
	...
	if test "$OCAMLOPTDOTOPT" != "no"; then
	   TMPVERSION=`$OCAMLOPTDOTOPT -v | tr -d '\r' | sed -n -e 's|.*version* *\(.*\)$|\1|p' `
	   if test "$TMPVERSION" != "$OCAMLVERSION" ; then
  ...
  if test "$OCAMLDEP" != "no" ; then
    ...
    if test "$OCAMLDEPDOTOPT" != "no"; then
      TMPVERSION=`$OCAMLDEPDOTOPT -version | tr -d '\r' | sed -n -e 's|.*version* *\(.*\)$|\1|p' `
      if test "$TMPVERSION" != "$OCAMLVERSION" ; then
...
[dnl
  ...
  if test "$CAMLP4" != "no"; then
     TMPVERSION=`$CAMLP4 -v 2>&1| tr -d '\r' | sed -n -e 's|.*version *\(.*\)$|\1|p'`
     if test "$TMPVERSION" != "$OCAMLVERSION" ; then
