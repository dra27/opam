  ...
  prepare)
    ...
    if [ "$TRAVIS_OS_NAME" = "windows" -a $# -eq 0 ] ; then
      if [ ! -e ~/local/setup-x86.exe ] ; then
        ...
        ~/local/setup-x86.exe --quiet-mode --no-shortcuts --no-startmenu --no-desktop --only-site --root "$(cygpath -w ~/local)" --site "$CYG_MIRROR" --local-package-dir "$(cygpath ~/local/setup-cache)" --packages make,patch,curl,diffutils,tar,unzip,gcc-g++,flexdll,m4,git > /dev/null
